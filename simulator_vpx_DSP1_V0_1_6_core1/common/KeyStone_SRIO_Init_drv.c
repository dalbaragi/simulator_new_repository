...
void KeyStone_SRIO_Init(SRIO_Config * srio_cfg)
{
	...
	// for srio switch bridge, 2015_8_26
	while(rio_local_config());
	...
	// printf srio cfg info, 2015_8_26
	printf ("RapidIO port is trained OK \n");

	if((gpSRIO_regs->RIO_SP[0].RIO_SP_CTL & PWIDTH_MASK) == PWIDTH1L0)
		printf("RapidIO port trained 1x on lane 0\n");

	if((gpSRIO_regs->RIO_SP[0].RIO_SP_CTL & PWIDTH_MASK) == PWIDTH1L2)
		printf("RapidIO port trained 1x on lane 2\n");

	if((gpSRIO_regs->RIO_SP[0].RIO_SP_CTL & PWIDTH_MASK) == PWIDTH4)
		printf("RapidIO port trained 4x \n");
	...
	SRIO_Packet_Type packetType)
{
	...
	*uipData= _swap4(_packlh2(*uipData, *uipData));	
    ...
    lsuTransfer.idSize = 0;
    lsuTransfer.srcIDMap = 0;
    lsuTransfer.hopCount = 0; // 2015_8_26, maintenance for local device
...
// 2015_8_26, maintenance for bridge
Int32 KeyStone_SRIO_Maintenance_hop(Uint32 uiPort, Uint32 uiLSU_No,
	Uint32 uiDestID, Uint32 uiOffSet, Uint32 uiLocalAddress,
	SRIO_Packet_Type packetType, Uint8 hop_count)
{
	SRIO_LSU_Transfer lsuTransfer;
	Uint32 uiCompletionCode;
#ifdef _LITTLE_ENDIAN
...
//    lsuTransfer.idSize = 0;
    ...
    lsuTransfer.srcIDMap = 0;
    lsuTransfer.hopCount = hop_count; // 2015_8_26, maintenance for bridge
